...
library(rnaturalearth) #downloads one shapefile - this dependency can be removed by download data from github rep
...
```{r}
ggplot(data = volume %>% group_by(ftname_class) %>% count()) +
  geom_col(aes(x = n, y = ftname_class, fill = c("gray30", "gray30", "gray30", "gray30", "gray30", "gray30", "gray90", "gray30", "gray90", "gray30")), show.legend = FALSE) +
...
`r volume %>% filter(ftname %in% c("geoaddress_full", "parcel_full")) %>% summarize(sum(quantidade_feicoes_bruta)) %>% sum() %>% format(scientific = FALSE, big.mark = ".")` endereços completos, compartilhados por `r volume %>% filter(ftname %in% c("geoaddress_full", "parcel_full")) %>% group_by(legalname) %>% tally() %>% nrow()` parceiros

`r volume %>% filter(shortname != "OSM" & ftname %in% c("geoaddress_full", "parcel_full")) %>% summarize(sum(quantidade_feicoes_bruta)) %>% sum() %>% format(scientific = FALSE, big.mark = ".")` endereços completos com licença aberta, compartilhados por `r volume %>% filter(shortname != "OSM" & ftname %in% c("geoaddress_full", "parcel_full")) %>% group_by(legalname) %>% tally() %>% nrow()` parceiros

`r volume %>% filter(shortname != "OSM" & ftname %in% c("geoaddress_full", "parcel_full") & license_is_explicit == "yes") %>% summarize(sum(quantidade_feicoes_bruta)) %>% sum() %>% format(scientific = FALSE, big.mark = ".")` endereços completos com licença aberta e explícita, compartilhados por `r volume %>% filter(shortname != "OSM" & ftname %in% c("geoaddress_full", "parcel_full") & license_is_explicit == "yes") %>% group_by(legalname) %>% tally() %>% nrow()` parceiros
...
volume %>% 
  ...
  select(legalname, ftname_class, quantidade_feicoes_bruta, license, license_is_explicit) %>%
  mutate(quantidade_feicoes_bruta = format(quantidade_feicoes_bruta, big.mark = ".")) %>%
  setNames(c("Donor name", "Type", "Number of addresses", "License type", "License is explicit?")) %>%
  arrange(`Donor name`) %>%
